...
class EnhancedComparator extends Comparator
{
    ...
    public function compare(Schema $fromSchema, Schema $toSchema)
    {
        ...
        foreach ( $toSchema->getTables() AS $tableName => $table ) {
            if ( !$fromSchema->hasTable($tableName) ) {
                ...
                $tableDifferences = $this->diffTable( $fromSchema->getTable($tableName), $table, $foreignKeysToTable );
     ...
     * @param Table $table1
     * @param Table $table2
	 * @param array $foreignKeysToTable
     *
     * @return bool|EnhancedTableDiff
     */
    public function diffTable(Table $table1, Table $table2, array $foreignKeysToTable)
    {
        $changes = 0;
        ...
        foreach ( $table1Columns as $columnName => $column ) {
            if ( $table2->hasColumn($columnName) ) {
                ...
                if (count($changedProperties) ) {
                    ...
                    if (isset($foreignKeysToTable[$tableName]))
					{
						foreach ($foreignKeysToTable[$tableName] as $constraint)
						{
     ...
     */
    private function detectColumnRenamings(EnhancedTableDiff $tableDifferences)
    {
